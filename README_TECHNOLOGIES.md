...
| `scripts/setup-github-secrets_variables.ps1`     | `ignore/環境情報.md` から GitHub Secrets/Variables を一括作成 | セットアップ時に手動実行し、規定値のシークレットと環境変数を投入。`gh secret` / `gh variable` をラップして一括登録                                                                                                                     |
