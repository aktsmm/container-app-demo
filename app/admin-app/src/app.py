  ...
  <script>
    ...
    async function loadBackups() {
      ...
      try {
        ...
        let html = `<p>合計: ${data.count} ファイル</p><table><thead><tr><th>ファイル名</th><th>サイズ (KB)</th><th>最終更新</th></tr></thead><tbody>`;
        data.backups.forEach(b => {
          const sizeKB = Math.round(b.size / 1024);
          const date = b.lastModified ? new Date(b.lastModified).toLocaleString('ja-JP') : 'N/A';
          html += `<tr><td>${b.name}</td><td>${sizeKB}</td><td class="time">${date}</td></tr>`;
        });
        html += '</tbody></table>';
