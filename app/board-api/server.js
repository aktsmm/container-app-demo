...
if (!DB_ENDPOINT || !DB_APP_USERNAME || !DB_APP_PASSWORD) {
  ...
  console.error('必須環境変数(DB_ENDPOINT/DB_APP_USERNAME/DB_APP_PASSWORD)が未設定です');
  process.exit(1);
}
...
async function ensureDatabaseAndTable() {
  ...
  await rootConn.query('CREATE DATABASE IF NOT EXISTS boardapp CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci');
  await rootConn.end();
...
app.get('/api/posts', async (_, res) => {
  try {
    const conn = await pool.getConnection();
    const [rows] = await conn.query('SELECT id, author, message, created_at FROM posts ORDER BY created_at DESC');
    conn.release();
    res.json(rows.map(r => ({
      id: r.id,
      author: r.author,
...
app.post('/api/posts', async (req, res) => {
  ...
  try {
    ...
    const conn = await pool.getConnection();
    await conn.query('INSERT INTO posts (id, author, message, created_at) VALUES (?, ?, ?, ?)', [
      id,
      author,
      message,
      createdAt,
    ...
    conn.release();
    res.status(201).json({ id, author, message, createdAt: createdAt.toISOString() });
  } catch (err) {
