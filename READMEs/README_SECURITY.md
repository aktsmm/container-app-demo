...
| **GitGuardian**  | Security Scan / 2️⃣ Board App Build & Deploy | GitGuardian API (ggshield) による 400+ パターンのシークレット検出。Board ワークフローではビルド前スキャンとして動作し、`vars.GITGUARDIAN_API_KEY` 設定時のみ実行 |
...
> GitGuardian の実行には GitHub Variables へ `GITGUARDIAN_API_KEY`（`scan` / `incident:read` / `incident:write` スコープ付き PAT）を登録する必要があります。未設定の場合は GitGuardian ジョブのみスキップされます。また `workflow_dispatch` で `skip_board_scans=true` を渡すと、Board 向けの Gitleaks/GitGuardian/Trivy(IaC) をまとめてスキップでき、Step Summary にも「手動入力によりスキップした」旨が記録されます。
