...
| `scripts/setup-github-secrets_variables.ps1` | `$AzureCredentials` を gh CLI で一括登録        | `$DefaultRepo` と `$AzureCredentials` の編集が必須。プレースホルダーのまま実行するとエラーで制御される。`scripts/create-github-actions-sp.ps1` の出力値を `$AzureCredentials` に転記してから運用。`-DryRun` でプラン確認後に本番実行。  |
