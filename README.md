...
2. **GitHub Secrets/Variables を登録** – `scripts/setup-github-secrets_variables.ps1` の先頭で `$DefaultRepo` と `$AzureCredentials` を編集し、スクリプトを実行して GitHub リポジトリへ投入します。未編集の場合はエラーで制御されます。
